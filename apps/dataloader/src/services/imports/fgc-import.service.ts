...
const FGC_GTFS_ARCHIVE_URL = "https://www.fgc.cat/google/google_transit.zip";

const FGC_STOP_PREFIX = "FGCS:";
const FGC_PLATFORM_PREFIX = "FGCP:";
const FGC_ROUTE_PREFIX = "FGCR:";

const LOCATION_TYPE_PLATFORM = new Set(["0", "4"]);
const LOCATION_TYPE_ENTRANCE = "2";
const EMPTY_LOCATION_TYPE = "0";

const toFgcStopId = (gtfsStopId: string): string =>
    `${FGC_STOP_PREFIX}${gtfsStopId}`;
const toFgcPlatformId = (gtfsStopId: string): string =>
    `${FGC_PLATFORM_PREFIX}${gtfsStopId}`;
const toFgcRouteId = (gtfsRouteId: string): string =>
    `${FGC_ROUTE_PREFIX}${gtfsRouteId}`;
...
export type FgcSnapshot = StopSnapshot & {
...
export class FgcImportService {
    async getFgcSnapshot(): Promise<FgcSnapshot> {
        const response = await fetchWithTimeout(FGC_GTFS_ARCHIVE_URL);

        if (!response.ok) {
            throw new Error(
                `Failed to fetch FGC GTFS archive: ${response.status} ${response.statusText}`,
            );
        }
        ...
        const getFile = (path: string): Promise<string> => {
            ...
            if (!file) {
                throw new Error(`FGC GTFS archive is missing '${path}'`);
            }
    ...
    private async buildSnapshot({
        ...
        const fgcRoutes = rawRoutes.map((row) => this.parseRoute(row));
        const fgcRouteIds = new Set(fgcRoutes.map((route) => route.id));
        const fgcTrips = rawTrips
            .map((row) => this.parseTrip(row))
            .filter((trip) => fgcRouteIds.has(trip.routeId));
        const fgcTripById = new Map(
            fgcTrips.map((trip) => [trip.id, trip] as const),
        );
        const fgcStopTimesByTripId = new Map<string, ParsedStopTime[]>();

        for (const stopTime of rawStopTimes.map((row) =>
            this.parseStopTime(row),
        )) {
            if (!fgcTripById.has(stopTime.tripId)) {
                continue;
            }

            const tripStopTimes =
                fgcStopTimesByTripId.get(stopTime.tripId) ?? [];

            tripStopTimes.push(stopTime);
            fgcStopTimesByTripId.set(stopTime.tripId, tripStopTimes);
        }
        ...
        for (const tripStopTimes of fgcStopTimesByTripId.values()) {
        ...
        for (const trip of fgcTrips) {
            const tripStopTimes = (
                fgcStopTimesByTripId.get(trip.id) ?? []
            ).sort((left, right) => left.stopSequence - right.stopSequence);
            const platformIds = tripStopTimes
                .map((stopTime) => toFgcPlatformId(stopTime.stopId))
            ...
            for (const platformId of platformIds) {
                platformById
                    .get(platformId)
                    ?.routeIds.add(toFgcRouteId(trip.routeId));
            }
        ...
        const gtfsRoutes = fgcRoutes.map((route) => {
        ...
        const gtfsStationEntrances = logicalStops.flatMap((stop) =>
            stop.entrances.map((entrance) => ({
                id: `${FGC_STOP_PREFIX}entrance:${entrance.id}`,
                feedId: GtfsFeedId.BARCELONA,
                stopId: stop.id,
        ...
        const gtfsPersistenceSnapshot = buildGtfsPersistenceSnapshot({
            ...
            mapRouteId: toFgcRouteId,
            mapStopId: toFgcPlatformId,
        });
    ...
    private buildLogicalStops({
        ...
        for (const logicalStopSourceId of [...logicalStopIds].sort((a, b) =>
            ...
            const logicalStopId = toFgcStopId(logicalStopSourceId);
            const platformSeeds =
            ...
            const platforms: LogicalPlatform[] = platformSeeds
                .map((platformStop) => ({
                    id: toFgcPlatformId(platformStop.id),
    ...
    private parseStop(row: Record<string, string>): ParsedStop {
        ...
        if (!Number.isFinite(latitude) || !Number.isFinite(longitude)) {
            throw new Error(
                `Invalid FGC GTFS stop coordinates for '${parsed.stop_id}'`,
            );
        }
    ...
    private parseStopTime(row: Record<string, string>): ParsedStopTime {
        ...
        if (!Number.isInteger(stopSequence)) {
            throw new Error(
                `Invalid FGC GTFS stop sequence '${parsed.stop_sequence}'`,
            );
        }
