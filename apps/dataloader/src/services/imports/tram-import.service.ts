...
const TRAM_GTFS_ARCHIVE_URL = "https://opendata.tram.cat/GTFS/zip/TBS.zip";

const TRAM_STOP_PREFIX = "TRMS:";
const TRAM_PLATFORM_PREFIX = "TRMP:";
const TRAM_ROUTE_PREFIX = "TRMR:";

const LOCATION_TYPE_PLATFORM = new Set(["0", "4"]);
const LOCATION_TYPE_ENTRANCE = "2";
const EMPTY_LOCATION_TYPE = "0";

const toTramStopId = (gtfsStopId: string): string =>
    `${TRAM_STOP_PREFIX}${gtfsStopId}`;
const toTramPlatformId = (gtfsStopId: string): string =>
    `${TRAM_PLATFORM_PREFIX}${gtfsStopId}`;
const toTramRouteId = (gtfsRouteId: string): string =>
    `${TRAM_ROUTE_PREFIX}${gtfsRouteId}`;
...
export type TramSnapshot = StopSnapshot & {
...
export class TramImportService {
    async getTramSnapshot(): Promise<TramSnapshot> {
        const response = await fetchWithTimeout(TRAM_GTFS_ARCHIVE_URL);

        if (!response.ok) {
            throw new Error(
                `Failed to fetch TRAM GTFS archive: ${response.status} ${response.statusText}`,
            );
        }
        ...
        const getFile = (path: string): Promise<string> => {
            ...
            if (!file) {
                throw new Error(`TRAM GTFS archive is missing '${path}'`);
            }
    ...
    private async buildSnapshot({
        ...
        const tramRoutes = rawRoutes.map((row) => this.parseRoute(row));
        const tramRouteIds = new Set(tramRoutes.map((route) => route.id));
        const tramTrips = rawTrips
            .map((row) => this.parseTrip(row))
            .filter((trip) => tramRouteIds.has(trip.routeId));
        const tramTripById = new Map(
            tramTrips.map((trip) => [trip.id, trip] as const),
        );
        const tramStopTimesByTripId = new Map<string, ParsedStopTime[]>();

        for (const stopTime of rawStopTimes.map((row) =>
            this.parseStopTime(row),
        )) {
            if (!tramTripById.has(stopTime.tripId)) {
                continue;
            }

            const tripStopTimes =
                tramStopTimesByTripId.get(stopTime.tripId) ?? [];

            tripStopTimes.push(stopTime);
            tramStopTimesByTripId.set(stopTime.tripId, tripStopTimes);
        }
        ...
        for (const tripStopTimes of tramStopTimesByTripId.values()) {
        ...
        for (const trip of tramTrips) {
            ...
            for (const platformId of platformIds) {
                platformById
                    .get(platformId)
                    ?.routeIds.add(toTramRouteId(trip.routeId));
            }
        ...
        const gtfsRoutes = tramRoutes.map((route) => {
        ...
        const gtfsStationEntrances = logicalStops.flatMap((stop) =>
            stop.entrances.map((entrance) => ({
                id: `${TRAM_STOP_PREFIX}entrance:${entrance.id}`,
                feedId: GtfsFeedId.BARCELONA,
                stopId: stop.id,
        ...
        const gtfsPersistenceSnapshot = buildGtfsPersistenceSnapshot({
            ...
            mapRouteId: toTramRouteId,
            mapStopId: toTramPlatformId,
        });
    ...
    private buildLogicalStops({
        ...
        for (const logicalStopSourceId of [...logicalStopIds].sort((a, b) =>
            ...
            const logicalStopId = toTramStopId(logicalStopSourceId);
            const platformSeeds =
            ...
            const platforms: LogicalPlatform[] = platformSeeds
                .map((platformStop) => ({
                    id: toTramPlatformId(platformStop.id),
        ...
        for (const route of tramRoutes) {
            ...
            for (const { pattern } of dominantPattern) {
                for (const [
                    ...
                    routeStops.push({
                        feedId: GtfsFeedId.BARCELONA,
                        routeId: toTramRouteId(route.id),
    ...
    private parseStop(row: Record<string, string>): ParsedStop {
        ...
        if (!Number.isFinite(latitude) || !Number.isFinite(longitude)) {
            throw new Error(
                `Invalid TRAM GTFS stop coordinates for '${parsed.stop_id}'`,
            );
        }
    ...
    private parseStopTime(row: Record<string, string>): ParsedStopTime {
        ...
        if (!Number.isInteger(stopSequence)) {
            throw new Error(
                `Invalid TRAM GTFS stop sequence '${parsed.stop_sequence}'`,
            );
        }
