...
const AMB_GTFS_ARCHIVE_URL =
    "https://www.ambmobilitat.cat/OpenData/google_transit.zip";

const AMB_STOP_PREFIX = "AMBS:";
const AMB_PLATFORM_PREFIX = "AMBP:";
const AMB_ROUTE_PREFIX = "AMBR:";

const LOCATION_TYPE_PLATFORM = new Set(["0", "4"]);
const LOCATION_TYPE_ENTRANCE = "2";
const EMPTY_LOCATION_TYPE = "0";

const toAmbStopId = (gtfsStopId: string): string =>
    `${AMB_STOP_PREFIX}${gtfsStopId}`;
const toAmbPlatformId = (gtfsStopId: string): string =>
    `${AMB_PLATFORM_PREFIX}${gtfsStopId}`;
const toAmbRouteId = (gtfsRouteId: string): string =>
    `${AMB_ROUTE_PREFIX}${gtfsRouteId}`;
...
export type AmbSnapshot = StopSnapshot & {
...
export class AmbImportService {
    async getAmbSnapshot(): Promise<AmbSnapshot> {
        const response = await fetchWithTimeout(AMB_GTFS_ARCHIVE_URL);

        if (!response.ok) {
            throw new Error(
                `Failed to fetch AMB GTFS archive: ${response.status} ${response.statusText}`,
            );
        }
        ...
        const getFile = (path: string): Promise<string> => {
            ...
            if (!file) {
                throw new Error(`AMB GTFS archive is missing '${path}'`);
            }
    ...
    private async buildSnapshot({
        ...
        const ambRoutes = rawRoutes.map((row) => this.parseRoute(row));
        const ambRouteIds = new Set(ambRoutes.map((route) => route.id));
        const ambTrips = rawTrips
            .map((row) => this.parseTrip(row))
            .filter((trip) => ambRouteIds.has(trip.routeId));
        const ambTripById = new Map(
            ambTrips.map((trip) => [trip.id, trip] as const),
        );
        const ambStopTimesByTripId = new Map<string, ParsedStopTime[]>();

        for (const stopTime of rawStopTimes.map((row) =>
            this.parseStopTime(row),
        )) {
            if (!ambTripById.has(stopTime.tripId)) {
                continue;
            }

            const tripStopTimes =
                ambStopTimesByTripId.get(stopTime.tripId) ?? [];

            tripStopTimes.push(stopTime);
            ambStopTimesByTripId.set(stopTime.tripId, tripStopTimes);
        }
        ...
        for (const tripStopTimes of ambStopTimesByTripId.values()) {
        ...
        for (const trip of ambTrips) {
            const tripStopTimes = (
                ambStopTimesByTripId.get(trip.id) ?? []
            ).sort((left, right) => left.stopSequence - right.stopSequence);
            const platformIds = tripStopTimes
                .map((stopTime) => toAmbPlatformId(stopTime.stopId))
            ...
            for (const platformId of platformIds) {
                platformById
                    .get(platformId)
                    ?.routeIds.add(toAmbRouteId(trip.routeId));
            }
        ...
        const gtfsRoutes = ambRoutes.map((route) => {
        ...
        const gtfsStationEntrances = logicalStops.flatMap((stop) =>
            stop.entrances.map((entrance) => ({
                id: `${AMB_STOP_PREFIX}entrance:${entrance.id}`,
                feedId: GtfsFeedId.BARCELONA,
                stopId: stop.id,
        ...
        const gtfsPersistenceSnapshot = buildGtfsPersistenceSnapshot({
            ...
            mapRouteId: toAmbRouteId,
            mapStopId: toAmbPlatformId,
        });
    ...
    private buildLogicalStops({
        ...
        for (const logicalStopSourceId of [...logicalStopIds].sort((a, b) =>
            ...
            const logicalStopId = toAmbStopId(logicalStopSourceId);
            const platformSeeds =
            ...
            const platforms: LogicalPlatform[] = platformSeeds
                .map((platformStop) => ({
                    id: toAmbPlatformId(platformStop.id),
    ...
    private parseStop(row: Record<string, string>): ParsedStop {
        ...
        if (!Number.isFinite(latitude) || !Number.isFinite(longitude)) {
            throw new Error(
                `Invalid AMB GTFS stop coordinates for '${parsed.stop_id}'`,
            );
        }
    ...
    private parseStopTime(row: Record<string, string>): ParsedStopTime {
        ...
        if (!Number.isInteger(stopSequence)) {
            throw new Error(
                `Invalid AMB GTFS stop sequence '${parsed.stop_sequence}'`,
            );
        }
