...
export class StopService {
  ...
  private async loadRailStopIds({
    ...
    const stopIds = uniqueSortedStrings(
      rows.flatMap(({ isMetro, routeName, stopId }) =>
        stopId && (isMetro || isRailRouteName(routeName)) ? [stopId] : [],
      ),
    );
  ...
  private async loadGraphQLStopsByIds(
    ...
    const platformIdsByStopId = await this.loadStopGraphQLPlatformIdsByStopIds(
  ...
  async getGraphQLByIds(ids: readonly string[]): Promise<StopGraphQLRecord[]> {
    const stopsById = await loadCachedBatch({
      cacheManager: this.cacheManager,
      getCacheKey: CACHE_KEYS.stop.getGraphQLById,
      keys: ids,
      loadMissing: async (missingIds) => this.loadGraphQLStopsByIds(missingIds),
  ...
  async getOneById(id: string): Promise<StopRecord | null> {
    return this.cacheManager.wrap(
      ...
      async () => {
        ...
        const entrancesByStopId = await this.loadStopEntrancesByStopIds([id]);
